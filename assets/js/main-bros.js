...
createApp({
  setup() {
      ...
      try {
        ...
        if (data.success) {
          searchResultsLP.value = data.data.result.map((property) => ({
            ...
            type: property.type,
            rooms:
              property.availableRooms[0]?.rooms.map((room) => ({
                type: room.type,
                available: room.available,
                max: room.max,
                board: room.board,
                price: room.price
                  ? (parseFloat(room.price) * 1.08).toFixed(2)
                  : "N/A",
                initialPrice: room.initialPrice
                  ? (parseFloat(room.initialPrice) * 1.08).toFixed(2)
                  : null,
                specialOffers: room.specialOffers || [],
                paymentTerms: room.paymentTerms || null,
                cancellationFees: room.cancellationFees || [],
              })) || [],
          }));

          // Filter properties by selected region, subregion, or location
          const filteredProperties = searchResultsLP.value.filter(
            (property) => {
          ...
          const allPrices = filteredProperties.flatMap((property) =>
            property.rooms.map((room) => parseFloat(room.price))
          );
    ...
    const generateStars = (rating) => {
      const starSrc =
        "/wp-content/plugins/bros-travel-plugin/assets/images/full-star.svg"; // Path to star image
      let starsHtml = "";
    ...
    const renderRoomDetails = (room) => {
      ...
      if (
        room.specialOffers &&
        Array.isArray(room.specialOffers) &&
    ...
    // --- Bezbedna logika za dugmad ---
    const cancellation = room.cancellationFees?.[0];
    const isNonRefundable =
        room.paymentTerms?.depositPercent === 100 &&
        cancellation?.percent === 100;

    let nonRefundableHTML = "";
    if (isNonRefundable) {
        nonRefundableHTML = `<span class="non-refundable-badge">Non refundable</span>`;
    }

    let termsHTML = "";
    if (room.paymentTerms) {
        termsHTML = `<span class="terms-badge">Terms of service</span>`;
    }
